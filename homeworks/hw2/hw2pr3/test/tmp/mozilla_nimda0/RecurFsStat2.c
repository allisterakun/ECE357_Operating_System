...
int totalSize, totalBlock, nlink1, nsymlink, problematic = 0;
...
int main(int argc, char *argv[]) {
	...
	printf("# of \'problematic\' directory entries %d.\n", problematic);
...
int ls(char *pathname){
    ...
    //int m = strlen(pathname)-1;
    /*
    for(m = strlen(pathname)-1; m >= 0; m--){
        if(pathname[m] == '/')
            break;
    }

	for(i = m; i < strlen(pathname) + 1; i++) {
		if(((42 < pathname[i] && pathname[i] < 59) || (63 < pathname[i] && pathname[i] < 91) || (93 < pathname[i] && pathname[i] < 96) || (96 < pathname[i] && pathname[i] < 127) || pathname[i] == 0)) {
            continue;
		} else {
...
*/
    ...
    if (dr == NULL) { // opendir returns NULL if couldn't open directory
        fprintf(stderr, "Could not open current directory %s: %s",pathname, strerror(errno) );
        return 0;
    }
